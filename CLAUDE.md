...
4. **Read protocol specs before implementing** - Read the current MCP specification before implementing any MCP feature. The server is **dual-era**: the legacy path (2025-11-25 and earlier) runs on FastMCP; the modern path targets **2026-07-28** (<https://modelcontextprotocol.io/specification/draft>, wire string `2026-07-28`) and is hand-implemented in [`virtual-library-mcp/modern/`](virtual-library-mcp/modern/). See [virtual-library-mcp/docs/mcp/11-protocol-2026-07-28.md](virtual-library-mcp/docs/mcp/11-protocol-2026-07-28.md). Do not assume the pinned FastMCP / `mcp` 1.x stack supports the modern era — it does not.
5. **Read the latest FastMCP documentation before implementing** - Read the current FastMCP docs for the version in pyproject.toml before touching the legacy path. The modern path deliberately does NOT use FastMCP — it reimplements the 2026-07-28 wire protocol from scratch for teaching.

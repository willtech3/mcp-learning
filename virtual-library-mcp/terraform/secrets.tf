...
# client, so the spec requires integrity protection. Even with one maximum
# instance, scale-to-zero and revision changes replace the process, so the
# HMAC key must be STABLE — a per-process random key would invalidate a retry
# after a recycle. The value is generated by the deploy workflow
